...
def _should_compile_optimizer_kernels() -> bool:
    val = os.environ.get("SCALABLE_OPTIM_COMPILE", "0").strip().lower()
    return val in {"1", "true", "yes", "on"}


def _maybe_compile(fn):
    if _should_compile_optimizer_kernels() and hasattr(torch, "compile"):
        # Optimizer kernels are called on many parameter shapes; dynamic shapes avoid
        # recompilation storms from static-shape guards.
...
adamw_step_fused = _maybe_compile(adamw_step_fused)
...
muon_step_fused = _maybe_compile(muon_step_fused)
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        # Some params can legitimately have grad=None on a step (unused branch, masking, etc.).
        grads = []
        valid = []
        for param in params:
        ...
        if bool(inactive_mask.any()):
            # Keep standard optimizer semantics for grad=None: do not update these params/states.
            saved_params = stacked_params[inactive_mask].clone()
            saved_momentum = momentum_buffer[inactive_mask].clone()
            saved_second = second_momentum_buffer[inactive_mask].clone()
        ...
        if bool(inactive_mask.any()):
            stacked_params[inactive_mask].copy_(saved_params)
            momentum_buffer[inactive_mask].copy_(saved_momentum)
            second_momentum_buffer[inactive_mask].copy_(saved_second)
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _reduce_muon(self, group: dict, world_size: int) -> dict:
        ...
        # Keep a validity mask so we can skip updates for grad=None params.
        grads = []
        valid = []
        for param in params:
            if param.grad is None:
        ...
        padded_valid_mask = torch.zeros(padded_num_params, device=device, dtype=torch.bool)
        padded_valid_mask[: len(params)] = valid_mask
    ...
    def _compute_muon(self, group: dict, info: dict, gather_list: list, rank: int) -> None:
        ...
        if num_owned > 0:
            ...
            owned_valid = valid_mask[start_idx : start_idx + num_owned]
            inactive_mask = ~owned_valid
            if bool(inactive_mask.any()):
                saved_params = stacked_owned[inactive_mask].clone()
                saved_momentum = state["momentum_buffer"][:num_owned][inactive_mask].clone()
                saved_second = state["second_momentum_buffer"][:num_owned][inactive_mask].clone()
            ...
            if bool(inactive_mask.any()):
                stacked_owned[inactive_mask].copy_(saved_params)
                state["momentum_buffer"][:num_owned][inactive_mask].copy_(saved_momentum)
                state["second_momentum_buffer"][:num_owned][inactive_mask].copy_(saved_second)
