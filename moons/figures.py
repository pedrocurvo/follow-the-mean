#!/usr/bin/env python3
"""Generate closed-form two-moons figures for the SPFM paper.

The two-moons experiment is a fully visualizable version of the database
steering mechanism. It uses sparse class references, a time-dependent kernel,
and database-conditioned sampling weights. No neural network is trained.

Figures:
1. Label posteriors sharpen over time.
2. Unconditional, soft-label, and hard-filter generation.
3. Velocity fields and trajectories under different database compositions.
4. Output class balance as a function of database composition.
...
# kNN evaluator used to score generated samples.
...
for M in M_list:
    print(f"[moons/figures] running DB-composition sweep for M={M}", flush=True)
    for frac in fracs:
        vals = []
